...
%% Generated on: May 11, 2021 3:33:51 AM
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% cic_fir_comp_coeff genearats CIC compensation filter coefficients
% using frequency sampling method.
%
%    cic_fir_comp_coeff(L, Fs, Fc, plot, is_fxp, B) calculates compensation
...
%
%    Examples:
%    cic_fir_comp_coeff(31,80e6,4e6,true,true,16);
%    cic_fir_comp_coeff(31,80e6,4e6,true,false)


function cic_fir_comp_coeff(L, Fs, Fc, plot, is_fxp, B)
try
	% Validate number of input arguments
	narginchk(0, 6);

	%%%%%% CIC filter parameters %%%%%%
	R = 2000;				%% Decimation factor
	...
	h = h/sum(h);                            %% Floating point coefficients, scaled it to 1
    Sfloat = sum(h);
    fprintf('sum(h) = %.8f\n', Sfloat);
	...
	filename = 'cic_fir_comp_coeff.txt';
	...
	if (is_fxp) % coefficients are fixed point
		hz = round(h*(2^(B-1)-1));               %% Quantization of filter coefficients
        Sint   = sum(double(hz));
        fprintf('sum(hz) = %d\n', Sint); 
