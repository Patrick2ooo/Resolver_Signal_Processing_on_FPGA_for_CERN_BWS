...
proc vhdl_compil {} {
    ...
    vcom -2008 $Path_VHDL/sinus_generator.vhd
...
proc sim_start {TESTCASE ANGLE_DEG_SIM} {
    ...
    if {[file exists wave.do] == 0} {
        add wave -position insertpoint sim:/rdc_tb/clk
        add wave -position insertpoint -radix decimal sim:/rdc_tb/in_if_inst/s1_in
        add wave -position insertpoint -radix decimal sim:/rdc_tb/in_if_inst/s2_in
        add wave -position insertpoint -radix decimal sim:/rdc_tb/dut/demodulated_sin_signal_s
        add wave -position insertpoint -radix decimal sim:/rdc_tb/dut/demodulated_cos_signal_s
        add wave -position insertpoint sim:/rdc_tb/angle_rad
        add wave -position insertpoint sim:/rdc_tb/rotor_angle_rad
...
if {$argc > 0} {
    if {[string compare $1 "all"] == 0} {
        ...
        puts "Command is: vsim -do "do ../scripts/rdc.do all/clean/noisy <angle in degree> <compile_IP>""
        quit -sim
    }
} else {
    puts "Default values"
